import { msg } from '@lingui/core/macro';
import { useLingui } from '@lingui/react';
import { Trans } from '@lingui/react/macro';

import { withPreviewI18n } from '../preview-i18n-wrapper';
import {
  EmailHeading,
  EmailParagraph,
  TemplateBaseLayout,
} from '../template-components/template-base-layout';

export type OrganisationLeaveEmailProps = {
  assetBaseUrl?: string;
  baseUrl?: string;
  memberName?: string;
  memberEmail?: string;
  organisationName?: string;
  organisationUrl?: string;
};

export const OrganisationLeaveEmailTemplate = ({
  memberName = 'John Doe',
  memberEmail = 'johndoe@example.com',
  organisationName = 'Organisation Name',
}: OrganisationLeaveEmailProps) => {
  const { _ } = useLingui();
  const previewText = _(msg`${memberName || memberEmail} ha dejado ${organisationName}`);

  return (
    <TemplateBaseLayout previewText={previewText}>
      <EmailHeading>
        <Trans>¡Hola!</Trans>
      </EmailHeading>

      <EmailParagraph>
        <Trans>
          <strong>{memberName || memberEmail}</strong> ({memberEmail}) ha dejado la organización{' '}
          <strong>{organisationName}</strong>.
        </Trans>
      </EmailParagraph>
    </TemplateBaseLayout>
  );
};

export default withPreviewI18n(OrganisationLeaveEmailTemplate);
